...
public static class HttpClientRegistrationExtensions
{
    ...
    )
    {
        ...
        services
            .AddHttpClient(HttpClientKeys.MorningStar)
            .SetHandlerLifetime(TimeSpan.FromMinutes(5))
            .ConfigureHttpClient(
                (sp, client) =>
                {
                    client.BaseAddress = new Uri(
                        configuration.GetValue<string>(ConfigurationKeys.MorningStarBaseUrl) ?? ""
                    );
                }
            )
        ...
        services
            .AddHttpClient(HttpClientKeys.MorningStarCenter)
            .SetHandlerLifetime(TimeSpan.FromMinutes(5))
            .ConfigureHttpClient(
                (sp, client) =>
                {
                    client.BaseAddress = new Uri(
                        configuration.GetValue<string>(ConfigurationKeys.MorningStarCenterBaseUrl)
                        ?? ""
                    );
                }
            )
