    ...
    where TEntity : class
{
    ...
    Task BulkUpsertAsync(IEnumerable<TEntity> entities, Expression<Func<TEntity, bool>> filter);
    Task UpsertAsync(string id, TEntity entity);
    Task ReplaceAsync(string id, TEntity entity);
    Task DeleteAsync(string id);
    Task DeleteByFilterAsync(Expression<Func<TEntity, bool>> filterExpression);
