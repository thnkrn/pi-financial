...
public class CuratedFilterController : ControllerBase
{
    ...
    {
        var curatedFilters = CsvHelperExtensions.ReadCsvData<CuratedFilter>(file.OpenReadStream());
        if (curatedFilters == null || !curatedFilters.Any())
        {
            return BadRequest(new ProblemDetails { Status = 400, Title = "No valid data found in the CSV file" });
        }
        _fileUtils.SaveFile(file);
        var response = await _requestHelper.ExecuteMarketDataManagementRequest<CreateCuratedFilterRequest, CreateCuratedFilterResponse>(
            new CreateCuratedFilterRequest(curatedFilters),
            _ => ModelState.IsValid
        );
