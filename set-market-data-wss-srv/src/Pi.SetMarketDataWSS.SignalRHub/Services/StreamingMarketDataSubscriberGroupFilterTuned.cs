        ...
        MarketStreamingResponse response)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                response.Code = marketStreamingResponse.Code ?? "200";
                response.Op = marketStreamingResponse.Op ?? "Streaming";
                response.Message = marketStreamingResponse.Message ?? string.Empty;
    ...
    private void CleanupOldConnections(object? state)
    {
        try
        {
            ...
            for (var i = 0; i < keysToRemove.Count; i += batchSize)
            {
                ...
                foreach (var connectionId in batch)
                    tasks.Add(RemoveSubscriptionAsync(connectionId));
