...
{
    private const string SetHighVolumeTopic = "set_stock_market_bid_offer";
    private const string TfexHighVolumeTopic = "tfex_stock_market_bid_offer";
    ...
    private readonly TransformBlock<ConsumeResult<TKey, TValue>, ConsumeResult<TKey, TValue>> _deduplicationBlock;
    private readonly CancellationTokenSource _internalCts = new();
    private readonly ConcurrentDictionary<string, ConsumeResult<TKey, TValue>> _latestMessageByKey = new();
    ...
    private readonly ConcurrentDictionary<TopicPartition, ConcurrentDictionary<long, bool>> _pendingOffsetsByPartition =
        new();

    // DataFlow blocks for processing pipeline
    private readonly TransformBlock<ConsumeResult<TKey, TValue>, ConsumeResult<TKey, TValue>> _preprocessBlock;

    // Store processed messages waiting to be committed - key is TopicPartitionOffset, value is whether it's fully processed and stored
    private readonly ConcurrentDictionary<TopicPartitionOffset, ProcessingStatus> _processedMessages = new();
    ...
    private long _totalDedupedMessages;
        ...
        ILogger<KafkaSubscriber<TKey, TValue>> logger)
    {
        ...
        _deduplication = _configuration.GetValue("KAFKA:DEDUPLICATION", true);

        // Special handling for high-volume topics
        if (!string.IsNullOrEmpty(_topic)
            && (_topic.Equals(SetHighVolumeTopic, StringComparison.OrdinalIgnoreCase)
                || _topic.Equals(TfexHighVolumeTopic, StringComparison.OrdinalIgnoreCase)))
        {
            maxConcurrentTasks = (int)Math.Round(maxConcurrentTasks / 2.0);
            _logger.LogInformation("Using optimized configuration for high-volume bid-offer topic");
        }
        ...
        // Block for message deduplication
        _deduplicationBlock = new TransformBlock<ConsumeResult<TKey, TValue>, ConsumeResult<TKey, TValue>>(
            message =>
            {
                // Simple optimization: only process the latest message for each key
                var messageKey = message.Message.Key?.ToString();
                if (messageKey != null && _deduplication)
                    _latestMessageByKey.AddOrUpdate(
                        messageKey,
                        message,
                        (_, existing) => message.Offset.Value > existing.Offset.Value ? message : existing);
            ...
            new ExecutionDataflowBlockOptions
            {
                ...
                // Deduplication must be sequential
        ...
        _processingBlock = new ActionBlock<ConsumeResult<TKey, TValue>>(
            async message =>
            {
                try
                {
                    // Skip if not the latest message for this key (deduplication)
                    var messageKey = message.Message.Key?.ToString();
                    if (_deduplication && messageKey != null &&
                        _latestMessageByKey.TryGetValue(messageKey, out var latestMessage) &&
                        latestMessage.Offset.Value > message.Offset.Value)
                    {
                        // This is an older message for this key, skip processing
                        Interlocked.Increment(ref _totalDedupedMessages);

                        // Don't track offsets for deduped messages until we confirm
                        // the newest one processed successfully
                        return;
                    }
                    ...
                    try
                    {
                        ...
                        // Initialize processing status with both flags as false
                        if (!_processedMessages.TryGetValue(tpo, out var processingStatus))
                        {
                            processingStatus = new ProcessingStatus { Processed = false, StoredInRedis = false };
                            _processedMessages.TryAdd(tpo, processingStatus);
                        }

                        // Process the message with updated interface - this now returns a tuple indicating both processing and storage success
                        var (processSuccess, storageSuccess) =
                            await messageHandler.HandleAsync(message.Message).ConfigureAwait(false);
                        ...
                        if (processSuccess)
                        {
                            ...
                            if (_processedMessages.TryGetValue(tpo, out processingStatus))
                            {
                                processingStatus.Processed = true;
                                processingStatus.StoredInRedis = storageSuccess;
                            }

                            // Only track offset for commit if both processing and storage were successful
                            if (storageSuccess)
                                // Track offset after successful processing and storage
                                TrackOffset(message);
                            else
                                _logger.LogWarning(
                                    "Message processing succeeded but storage failed for Topic={Topic}, Partition={Partition}, Offset={Offset}",
        ...
        _preprocessBlock.LinkTo(_deduplicationBlock, new DataflowLinkOptions { PropagateCompletion = true });
        _deduplicationBlock.LinkTo(_processingBlock, new DataflowLinkOptions { PropagateCompletion = true });
    ...
    public async Task UnsubscribeAsync()
    {
        ...
        try
        {
            ...
            await Task.WhenAll(
                ...
                _deduplicationBlock.Completion,
                _processingBlock.Completion
            ).WaitAsync(TimeSpan.FromMinutes(30));
        }
    ...
    private sealed class ProcessingStatus
    {
        ...
        public bool StoredInRedis { get; set; }

        public bool IsFullyProcessed => Processed && StoredInRedis;
    }
                ...
                _consumer = new ConsumerBuilder<TKey, TValue>(config)
                    ...
                    .SetPartitionsRevokedHandler((_, partitions) =>
                    {
                        ...
                        try
                        {
                            ...
                            {
                                // Modified: Only commit messages that are fully processed and stored
                                var fullyProcessedOffsets = _processedMessages
                                    .Where(kv => kv.Value.IsFullyProcessed)
                                    .Select(kv => kv.Key)
                                    .ToList();

                                if (fullyProcessedOffsets.Count > 0)
                                    try
                                    {
                                        _consumer.Commit(fullyProcessedOffsets);
                                        _logger.LogInformation(
                                            "Committed {Count} fully processed offsets during re-balance",
                                            fullyProcessedOffsets.Count);
                                    }
    ...
    // Track all offsets per partition to ensure correct ordering
    private void TrackOffset(ConsumeResult<TKey, TValue> consumeResult)
    {
        // Get or create the dictionary for this partition
        var offsetsByPartition = _pendingOffsetsByPartition.GetOrAdd(
            consumeResult.TopicPartition,
            _ => new ConcurrentDictionary<long, bool>());

        // Add this offset to the partition's tracked offsets
        offsetsByPartition.TryAdd(consumeResult.Offset.Value + 1, true);
    }
    ...
    private async Task<bool> CommitPendingOffsetsAsync(bool isShutdown)
    {
        ...
        try
        {
            ...
            if (_pendingOffsetsByPartition.IsEmpty)
            ...
            // For each partition, find contiguous ranges of offsets that are safe to commit
            foreach (var partitionEntry in _pendingOffsetsByPartition)
            {
                var partition = partitionEntry.Key;
                var offsetDictionary = partitionEntry.Value;

                if (offsetDictionary.Count == 0)
                    continue;

                // Find the highest offset that can be safely committed (all previous offsets are processed)
                long? commitOffset = null;
                var sortedOffsets = offsetDictionary.Keys.OrderBy(k => k).ToList();

                // Check for contiguous range from the beginning
                foreach (var currentOffset in sortedOffsets)
                {
                    var tpo = new TopicPartitionOffset(partition, new Offset(currentOffset));

                    // Only include this offset if it's marked as fully processed and stored
                    if (_processedMessages.TryGetValue(tpo, out var status) && status.IsFullyProcessed)
                    {
                        commitOffset = currentOffset;

                        // Remove this offset from tracking as we'll commit it
                        offsetDictionary.TryRemove(currentOffset, out _);
                    }
                    else
                    {
                        // Found an offset that's not fully processed, can't commit beyond this point
                        break;
                    }
                }
                ...
                if (commitOffset.HasValue)
                    offsetsToCommit.Add(new TopicPartitionOffset(
                        partition,
                        new Offset(commitOffset.Value)));
            }

            // Remove empty partition entries
            foreach (var partition in _pendingOffsetsByPartition.Where(p => p.Value.IsEmpty).Select(p => p.Key)
                         .ToList())
                _pendingOffsetsByPartition.TryRemove(partition, out _);

            if (offsetsToCommit.Count == 0)
            {
                _logger.LogDebug("No offsets ready to commit - waiting for more messages to be fully processed");
                return false;
            }
            ...
            try
            {
                ...
                _logger.LogInformation("Committed {Count} offsets after {TimeSinceLastCommit}ms",
                    offsetsToCommit.Count, timeSinceLastCommit);

                // After successful commit, remove these offsets from the processed messages tracking
                foreach (var offset in offsetsToCommit) _processedMessages.TryRemove(offset, out _);
    ...
    private void LogPerformanceMetrics(object? state)
    {
        ...
        var deduped = Interlocked.Read(ref _totalDedupedMessages);
        ...
        _logger.LogInformation(
            ...
            _deduplicationBlock.InputCount,
        ...
        Interlocked.Exchange(ref _totalDedupedMessages, 0);
        ...
        if (disposing)
        {
            ...
            try
            {
                ...
                if (_consumer != null)
                    try
                    {
                        // Collect all offsets that are fully processed
                        var fullyProcessedOffsets = _processedMessages
                            .Where(kv => kv.Value.IsFullyProcessed)
                            .Select(kv => kv.Key)
                            .ToList();
                        ...
                        {
                            _consumer.Commit(fullyProcessedOffsets);
                            _logger.LogInformation("Committed {Count} fully processed offsets during shutdown",
                                fullyProcessedOffsets.Count);
                        }
                    }
                    catch (Exception ex)
