...
import {
  getStateResources,
  isSupportedState,
} from '@libs/step-function-local-helper';
import type {
  AWS,
...
import * as StateMachines from './src/stateMachines';
...
export const serviceName = 'step-functions';
export const stepFunctionLocalAccountId = 101010101010;
export const stepFunctionLocalRegion = 'ap-southeast-1';
const serverlessConfiguration: ServerlessWithStepFunctions = {
  ...
  stepFunctions: {
    stateMachines: {
      ...Object.entries(StateMachines)
        .filter(([machineName, machineConfig]) => {
          const result =
            // NOTE: Check is supported state when in debug mode only, otherwise the statemachine will be disabled by default when deploying to staging or production
            process.env.AWS_ENVIRONMENT === 'production' ||
            process.env.DEBUG_LOCAL !== 'true' ||
            isSupportedState(machineConfig.definition.States);
          if (!result)
            console.log(
              `Unsupported Step Function Local StateMachine: ${machineName}`
            );
          return result;
        })
        .reduce(
  ...
  custom: {
    stepFunctionsLocal: {
      accountId: stepFunctionLocalAccountId,
      region: stepFunctionLocalRegion,
      stepFunctionsEndpoint: 'http://127.0.0.1:8083',
      lambdaEndpoint: 'http://127.0.0.1:3002',
      TaskResourceMapping: {
        ...Object.entries(StateMachines).reduce(
          (res, [_, machineConfig]) => ({
            ...res,
            ...getStateResources(machineConfig.definition.States).reduce(
              (subRes, stateKey) => ({
                ...subRes,
                [stateKey.state]: stateKey.Resource,
              }),
              {}
            ),
          }),
