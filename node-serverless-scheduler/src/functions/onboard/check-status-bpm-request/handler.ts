...
const checkStatusBpmRequest: Handler = async () => {
  try {
    const { onboardServiceHost } = await getConfig();

    const response = await got
      .post(`${onboardServiceHost}/internal/tasks/check-status-bpm-request`, {
        headers: {
...
export const main = middyfy(checkStatusBpmRequest);
