...
const updateNdidExpiredRequest: Handler = async () => {
  try {
    const { onboardServiceHost } = await getConfig();

    const response = await got
      .post(
        `${onboardServiceHost}/internal/tasks/update-ndid-expired-request`,
        {
          headers: {
...
export const main = middyfy(updateNdidExpiredRequest);
