...
const submitOpenAccountRequest: Handler = async () => {
  try {
    const { onboardServiceHost } = await getConfig();

    const response = await got
      .post(
        `${onboardServiceHost}/internal/tasks/submit-open-account-request`,
        {
          headers: {
...
export const main = middyfy(submitOpenAccountRequest);
