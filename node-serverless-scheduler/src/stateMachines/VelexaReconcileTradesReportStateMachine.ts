    ...
    MapFileFromS3: {
      ...
      ItemProcessor: {
        ...
        States: {
          InsertToDb: {
            Type: 'Task',
            Resource: { 'Fn::GetAtt': ['insertVelexaTradesReport', 'Arn'] },
            End: true,
          },
        },
      },
...
export const velexaReconcileTradesReport = {
  events: [
    {
      schedule: {
        rate: 'cron(15 1 * * ? *)', // Run 8:15 AM BKK Time Everyday
        enabled: process.env.AWS_ENVIRONMENT === 'production',
      },
    },
    {
      http: {
        path: 'reconcile/ge/trades',
        method: 'POST',
        request: {
          schemas: {
            'application/json': {
  ...
  name: `pi-${process.env.AWS_ENVIRONMENT}-VelexaReconcileTradesReport`,
  definition,
};
