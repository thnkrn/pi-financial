...
const settings = {
  ...
  reportName: 'Velexa Trades Vat',
  reportPrefix: 'QLO7111_report_trades_VAT',
  reportBucket: `backoffice-reports-${process.env.AWS_ENVIRONMENT}`,
  reportDir: 'velexa/trades-vat/',
    ...
    MapFileFromS3: {
      ...
      ItemProcessor: {
        ...
        States: {
          InsertToDb: {
            Type: 'Task',
            Resource: { 'Fn::GetAtt': ['insertVelexaTradeVatReport', 'Arn'] },
            End: true,
          },
        },
      },
...
export const velexaReconcileTradeVatReport = {
  events: [
    {
      schedule: {
        rate: 'cron(30 1 * * ? *)', // Run 8:30 AM BKK Time Everyday
        enabled: process.env.AWS_ENVIRONMENT === 'production',
      },
    },
    {
      http: {
        path: 'reconcile/ge/trade-vat',
        method: 'POST',
        request: {
          schemas: {
            'application/json': {
  ...
  name: `pi-${process.env.AWS_ENVIRONMENT}-VelexaReconcileTradeVatReport`,
  definition,
};
