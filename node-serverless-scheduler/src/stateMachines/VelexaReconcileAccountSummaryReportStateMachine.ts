...
const settings = {
  ...
  reportName: 'Velexa Account Summary',
  reportPrefix: 'QLO7111_account_summary',
  reportBucket: `backoffice-reports-${process.env.AWS_ENVIRONMENT}`,
  reportDir: 'velexa/account-summary/',
      ...
      ItemProcessor: {
        ...
        States: {
          InsertToDb: {
            Type: 'Task',
            Resource: {
              'Fn::GetAtt': ['insertVelexaAcoountSummaryReport', 'Arn'],
            },
            End: true,
          },
        },
...
export const velexaReconcileAccountSummaryReport = {
  events: [
    {
      schedule: {
        rate: 'cron(0 1 * * ? *)', // Run 8:00 AM BKK Time Everyday
        enabled: process.env.AWS_ENVIRONMENT === 'production',
      },
    },
    {
      http: {
        path: 'reconcile/ge/account-summary',
        method: 'POST',
        request: {
          schemas: {
            'application/json': {
  ...
  name: `pi-${process.env.AWS_ENVIRONMENT}-VelexaReconcileAccountSummaryReport`,
  definition,
};
