...
public sealed class KafkaSubscriptionService<TKey, TValue> : BackgroundService
{
        ...
        IHealthCheckReporter? healthReporter = null)
    {
        ...
        KafkaSubscriptionServiceConfig.MaxRetryDelayMs = configuration.GetValue(
            ConfigurationKeys.KafkaSubscriptionServiceMaxRetryDelayMs,
        ...
        _retryPolicy = Policy
            ...
            .WaitAndRetryAsync(
                ...
                (attempt, _) =>
                {
                    ...
                    var baseDelay = KafkaSubscriptionServiceConfig.InitialRetryDelayMs * Math.Pow(2, attempt - 1);
                    var jitter = new Random().Next(
                        (int)(baseDelay * 0.8),
                        (int)(baseDelay * 1.2));

                    return TimeSpan.FromMilliseconds(Math.Min(jitter, KafkaSubscriptionServiceConfig.MaxRetryDelayMs));
                },
