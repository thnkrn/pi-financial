...
namespace Pi.GlobalMarketDataWSS.SignalRHub.Tests;

public class StreamingMarketDataSubscriberGroupFilterTests
{
    private readonly Mock<IConnectionMultiplexer> _mockRedisConnection;
    private readonly Mock<IHubContext<StreamingHubGroupFilter>> _mockHubContext;
    private readonly Mock<IConfiguration> _mockConfiguration;
    private readonly Mock<ILogger<StreamingMarketDataSubscriberGroupFilter>> _mockLogger;
    ...
    public StreamingMarketDataSubscriberGroupFilterTests()
    {
        _mockRedisConnection = new Mock<IConnectionMultiplexer>();
        _mockHubContext = new Mock<IHubContext<StreamingHubGroupFilter>>();
        _mockConfiguration = new Mock<IConfiguration>();
        _mockLogger = new Mock<ILogger<StreamingMarketDataSubscriberGroupFilter>>();
    ...
    {
        // Arrange
        var subscriber = new StreamingMarketDataSubscriberGroupFilter(
    ...
    [Fact]
    public async Task UpdateSubscriptionAsync_ShouldAddToGroups()
    {
        // Arrange
        var subscriber = new StreamingMarketDataSubscriberGroupFilter(
            _mockRedisConnection.Object,
        ...
        // Assert
        _mockGroupManager.Verify(x => x.AddToGroupAsync("testConnection", "ReceiveMarketDataGroup_AAPL", It.IsAny<CancellationToken>()), Times.Once);
        _mockGroupManager.Verify(x => x.AddToGroupAsync("testConnection", "ReceiveMarketDataGroup_GOOGL", It.IsAny<CancellationToken>()), Times.Once);
        _mockLogger.Verify(
            x => x.Log(
    ...
    [Fact]
    public async Task RemoveSubscriptionAsync_ShouldRemoveFromGroups()
    {
        // Arrange
        var subscriber = new StreamingMarketDataSubscriberGroupFilter(
            _mockRedisConnection.Object,
        ...
        // Assert
        _mockGroupManager.Verify(x => x.RemoveFromGroupAsync("testConnection", "ReceiveMarketDataGroup_AAPL", It.IsAny<CancellationToken>()), Times.Once);
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Information,
                It.IsAny<EventId>(),
    ...
    [Fact]
    public async Task ExecuteAsync_ShouldSendUpdatesToMatchingGroups()
    {
        // Arrange
        var subscriber = new StreamingMarketDataSubscriberGroupFilter(
            _mockRedisConnection.Object,
        ...
        // Assert
        _mockHubClients.Verify(x => x.Group("ReceiveMarketDataGroup_AAPL"), Times.Once);
    ...
    public async Task StopAsync_ShouldCancelExecutingTask()
    {
        ...
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Information,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("StreamingMarketDataSubscriberGroupFilter is stopping")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.AtLeastOnce());
    }
