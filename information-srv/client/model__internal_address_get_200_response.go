/*
Information API

Contain Generic Information.

API version: 1.0
Contact: support@swagger.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// checks if the InternalAddressGet200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InternalAddressGet200Response{}

// InternalAddressGet200Response struct for InternalAddressGet200Response
type InternalAddressGet200Response struct {
	Detail *string          `json:"detail,omitempty"`
	Status *int32           `json:"status,omitempty"`
	Title  *string          `json:"title,omitempty"`
	Data   []AddressAddress `json:"data,omitempty"`
}

// NewInternalAddressGet200Response instantiates a new InternalAddressGet200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInternalAddressGet200Response() *InternalAddressGet200Response {
	this := InternalAddressGet200Response{}
	return &this
}

// NewInternalAddressGet200ResponseWithDefaults instantiates a new InternalAddressGet200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInternalAddressGet200ResponseWithDefaults() *InternalAddressGet200Response {
	this := InternalAddressGet200Response{}
	return &this
}

// GetDetail returns the Detail field value if set, zero value otherwise.
func (o *InternalAddressGet200Response) GetDetail() string {
	if o == nil || IsNil(o.Detail) {
		var ret string
		return ret
	}
	return *o.Detail
}

// GetDetailOk returns a tuple with the Detail field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InternalAddressGet200Response) GetDetailOk() (*string, bool) {
	if o == nil || IsNil(o.Detail) {
		return nil, false
	}
	return o.Detail, true
}

// HasDetail returns a boolean if a field has been set.
func (o *InternalAddressGet200Response) HasDetail() bool {
	if o != nil && !IsNil(o.Detail) {
		return true
	}

	return false
}

// SetDetail gets a reference to the given string and assigns it to the Detail field.
func (o *InternalAddressGet200Response) SetDetail(v string) {
	o.Detail = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *InternalAddressGet200Response) GetStatus() int32 {
	if o == nil || IsNil(o.Status) {
		var ret int32
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InternalAddressGet200Response) GetStatusOk() (*int32, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *InternalAddressGet200Response) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given int32 and assigns it to the Status field.
func (o *InternalAddressGet200Response) SetStatus(v int32) {
	o.Status = &v
}

// GetTitle returns the Title field value if set, zero value otherwise.
func (o *InternalAddressGet200Response) GetTitle() string {
	if o == nil || IsNil(o.Title) {
		var ret string
		return ret
	}
	return *o.Title
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InternalAddressGet200Response) GetTitleOk() (*string, bool) {
	if o == nil || IsNil(o.Title) {
		return nil, false
	}
	return o.Title, true
}

// HasTitle returns a boolean if a field has been set.
func (o *InternalAddressGet200Response) HasTitle() bool {
	if o != nil && !IsNil(o.Title) {
		return true
	}

	return false
}

// SetTitle gets a reference to the given string and assigns it to the Title field.
func (o *InternalAddressGet200Response) SetTitle(v string) {
	o.Title = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *InternalAddressGet200Response) GetData() []AddressAddress {
	if o == nil || IsNil(o.Data) {
		var ret []AddressAddress
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InternalAddressGet200Response) GetDataOk() ([]AddressAddress, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *InternalAddressGet200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []AddressAddress and assigns it to the Data field.
func (o *InternalAddressGet200Response) SetData(v []AddressAddress) {
	o.Data = v
}

func (o InternalAddressGet200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o InternalAddressGet200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Detail) {
		toSerialize["detail"] = o.Detail
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if !IsNil(o.Title) {
		toSerialize["title"] = o.Title
	}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	return toSerialize, nil
}

type NullableInternalAddressGet200Response struct {
	value *InternalAddressGet200Response
	isSet bool
}

func (v NullableInternalAddressGet200Response) Get() *InternalAddressGet200Response {
	return v.value
}

func (v *NullableInternalAddressGet200Response) Set(val *InternalAddressGet200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableInternalAddressGet200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableInternalAddressGet200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInternalAddressGet200Response(val *InternalAddressGet200Response) *NullableInternalAddressGet200Response {
	return &NullableInternalAddressGet200Response{value: val, isSet: true}
}

func (v NullableInternalAddressGet200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInternalAddressGet200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
