...
public class TimestampTests
{
    ...
    public void Timestamp_BytesConstructor_ValidatesCorrectly(long input, string expected)
    {
        ...
        Timestamp timestamp = new Timestamp(new ReadOnlySpan<byte>(bytes));
        Assert.Equal(expected, timestamp.ToString());
    }

    /*[Fact]
    public void ConvertToUnixTimestampNanoseconds_ValidInput_ReturnsCorrectValue()
    {
        // Arrange
        string input = "2024-03-01 03:51:09.954454552 UTC";
        long expectedNanoseconds = 1709265069954454552;

        // Act
        long result = Timestamp.ConvertToUnixTimestampNanoseconds(input);
