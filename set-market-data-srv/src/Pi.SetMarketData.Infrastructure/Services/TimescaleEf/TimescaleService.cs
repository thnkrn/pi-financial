    ...
    where TEntity : class
{
    ...
    private const string UpsertError = "Failed to upsert entity.";
    private const string BatchUpsertError = "Failed to Batch upsert entity.";
    ...
    public async Task UpsertAsync(TEntity entity, string tableName, params string[] conflictKeys)
    {
        try
        {
            await _repository.UpsertAsync(entity, tableName, conflictKeys);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, UpsertError);
            throw new TimescaleServiceException(UpsertError, ex);
        }
    }

    public async Task BatchUpsertAsync(IEnumerable<TEntity> entity, string tableName, params string[] conflictKeys)
    {
        try
        {
            await _repository.BatchUpsertAsync(entity, tableName, conflictKeys);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, BatchUpsertError);
            throw new TimescaleServiceException(BatchUpsertError, ex);
        }
    }
    ...
    public async Task UpsertBatchTechnicalIndicators(string timeframe, IEnumerable<TechnicalIndicators> technicalIndicators)
    {
        try
        {
            await _repository.UpsertBatchTechnicalIndicators(timeframe, technicalIndicators);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to upsert batch Technical Indicators with Symbol {Symbol}.", technicalIndicators.First().Symbol);
            throw new TimescaleServiceException(
                $"Failed to upsert batch Technical Indicators with Symbol {technicalIndicators.First().Symbol}.",
                ex
            );
        }
    }
    ...
    public async Task<DateTime> GetFirstCandle(string symbol, string venue)
    {
        try{
            ...
            _logger.LogError(ex, "Failed to get first candle time");
