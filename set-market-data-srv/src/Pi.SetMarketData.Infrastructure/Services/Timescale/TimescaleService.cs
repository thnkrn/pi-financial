...
using Pi.SetMarketData.Infrastructure.Interfaces.Timescale;

namespace Pi.SetMarketData.Infrastructure.Services.Timescale;
...
public class TimescaleService<TEntity> : ITimescaleService<TEntity> where TEntity : class
{
    ...
    public async Task UpsertAsync(TEntity entity)
    {
        ...
        {
            _logger.LogError(ex, "Failed to UpsertAsync entity.");
            throw new TimescaleServiceException("Failed to UpsertAsync entity.", ex);
        }
    }
}