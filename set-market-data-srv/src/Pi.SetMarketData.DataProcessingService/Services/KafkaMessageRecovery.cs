...
public interface IKafkaMessageRecovery<in T>
{
    Task<bool> HandleAsync(T message);
}

public class KafkaMessageRecovery : IKafkaMessageRecovery<Message<string, string>>
{
    ...
    private readonly ILogger<KafkaMessageRecovery> _logger;
    ...
    public KafkaMessageRecovery(
        ...
        ILogger<KafkaMessageRecovery> logger
    )
    {
    ...
    public async Task<bool> HandleAsync(Message<string, string> consumeMessage)
    {
        ...
        try
        {
            ...
            // Filter Message type 
            if (stockMessage.MessageType != ItchMessageType.i.ToString())
                return true;
        ...
        {
            stopwatch.Stop();
            _logger.LogInformation("HandleAsync(+Recovery) {MessageType} completed in {ElapsedMilliseconds} ms",
                stockMessage?.MessageType, stopwatch.ElapsedMilliseconds);
        }
    }
    ...
    private async Task<bool> HandleTradeTickerMessageAsync(string message)
    {
        ...
        var symbol = await _cacheServiceHelper.GetSymbolByOrderBookId(tradeTickerMessage.OrderbookId.Value);
        ...
        if (!filterSymbols.Contains(symbol)) 
            return true;
            ...
            switch (tableName.ToLowerInvariant())
            {
                case "realtime_market_data" when value is RealtimeMarketData realtimeMarketData:
                    using (var scope = _scopeFactory.CreateScope())
                    {
                        ...
                        _logger.LogInformation("+Recovery: {Symbol}, Venue: {Venue}, Date: {Date}",
