...
public static class MockMessageCreator
{
    ...
    {
        var bytes = Encoding.ASCII.GetBytes(value.PadRight(size));
    ...
    public static void AddDate(ref List<byte> messageBuilder, string date)
    {
        ...
        var dateInt = parsedDate.Year * 10000 + parsedDate.Month * 100 + parsedDate.Day;
        AddNumeric(ref messageBuilder, dateInt, 4);
    }

    public static void AddDate(ref List<byte> messageBuilder, uint dateInt)
    {
        AddNumeric(ref messageBuilder, dateInt, 4);
    }
    ...
    public static void AddTime(ref List<byte> messageBuilder, string time)
    {
        ...
        var timeInt = parsedTime.Hour * 10000 + parsedTime.Minute * 100 + parsedTime.Second;
    ...
    public static void AddTimestamp(ref List<byte> messageBuilder, string timeStamp)
    {
        ...
        var epochBegin = DateTime.Parse("1970-01-01");
        var durationSinceEpoch =
            parsedTimeStamp - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        var nanosecondsSinceEpoch =
            (long)(durationSinceEpoch.TotalSeconds * 1_000_000_000); // Convert from seconds to nanoseconds

        var nanosecondsBytes = BitConverter.GetBytes(nanosecondsSinceEpoch);
    ...
    public static void AddTimestamp(ref List<byte> messageBuilder, ulong nanosecondsSinceEpoch)
    {
        var nanosecondsBytes = BitConverter.GetBytes(nanosecondsSinceEpoch);
