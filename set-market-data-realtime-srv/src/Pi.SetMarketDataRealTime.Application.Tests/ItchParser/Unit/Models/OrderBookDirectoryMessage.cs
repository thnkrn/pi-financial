...
public class OrderBookDirectoryParserTests
{
    ...
    [Fact]
    private async Task Parse_OrderBookDirectoryMessage_VerifyToStringOutput()
    {
        // Arrange
        ...
        var orderBookDirectoryData = File.ReadAllBytes(binFilePath);

        // Act
        var orderBookDirectoryMessage =
            await itchParserService.Parse(orderBookDirectoryData) as OrderBookDirectoryMessage;
        ...
        Assert.Equal(
            DateTime.ParseExact(expectedParts[17], "yyyyMMdd", CultureInfo.InvariantCulture),
        ...
        Assert.Equal(
            expectedParts[31] == "0"
                ? DateTime.MinValue
                : DateTime.ParseExact(expectedParts[31], "yyyy-MM-dd", null),
        ...
        Assert.Equal(
            expectedParts[33] == "0"
                ? DateTime.MinValue
                : DateTime.ParseExact(expectedParts[33], "yyyyMMdd", CultureInfo.InvariantCulture),
        ...
        Assert.Equal(
            DateTime.ParseExact(expectedParts[42], "yyyyMMdd", CultureInfo.InvariantCulture),
