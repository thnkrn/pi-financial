...
public class TradeTickerMessageParserTests
{
    ...
    public async Task TradeTickerMessage_Constructor_SetsInputCorrectly(
        byte[] input,
        object[] result
    )
    {
        var output = await itchParserService.Parse(input) as TradeTickerMessage;
    ...
    [Fact]
    public async Task TradeTickerMessage_Constructor_SetsInputWithIncorrectFormat()
    {
        // Arrange
