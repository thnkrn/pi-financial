...
public partial class MarketStatisticMessageTests
{
    public static IEnumerable<object[]> MarketStatisticMessageTestData =>
        new List<object[]>
        {
            ...
            new object[]
            {
                new[] { (byte)'g' }, // Insufficient data for a complete message
    ...
    public async Task Parse_MarketStatisticMessage_VerifyBehavior(
        byte[] input,
        bool shouldSucceed,
        uint expectedNanos,
    ...
    )
    {
        ...
        if (shouldSucceed)
        {
            var message = await itchParserService.Parse(input) as MarketStatisticMessage;
