...
public class EncryptionService : IEncryptionCryptoProvider
{
    ...
    private byte[] AesEncrypt(byte[] input)
    {
        ...
        {
            _logger.LogError(e, "Unable to Encrypt with AES. {Message}", e.Message);
    ...
    private byte[] AesDecrypt(byte[] input)
    {
        ...
        {
            _logger.LogError(e, "Unable to Decrypt with AES. {Message}", e.Message);
