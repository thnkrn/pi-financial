...
namespace Pi.MarketData.Search.API.Converters;
...
public class StringDecimalJsonConverter : JsonConverter<decimal>
{
    ...
    {
        var text = Digit != null ? value.ToString($"F{Digit}", CultureInfo.InvariantCulture) : value.ToString(CultureInfo.InvariantCulture);
        if (string.IsNullOrWhiteSpace(text))
