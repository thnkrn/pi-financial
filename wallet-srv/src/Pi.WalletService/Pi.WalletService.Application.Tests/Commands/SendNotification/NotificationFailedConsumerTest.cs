...
public class NotificationFailedConsumerTest : ConsumerTest
{
    private readonly Mock<ILogger<NotificationFailedConsumer>> _logger;
    private readonly Mock<ITransactionQueriesV2> _transactionQueriesV2;
    private readonly Mock<INotificationService> _notificationService;

    public NotificationFailedConsumerTest()
    {
        _logger = new Mock<ILogger<NotificationFailedConsumer>>();
        _transactionQueriesV2 = new Mock<ITransactionQueriesV2>();
        _notificationService = new Mock<INotificationService>();

        Provider = new ServiceCollection()
            .AddMassTransitTestHarness(cfg => { cfg.AddConsumer<NotificationFailedConsumer>(); })
    ...
    public async void Should_Be_Throw_InvalidDataException_When_Transaction_Not_Found()
    {
        ...
        var payload = new DepositWithdrawFailedNotification(correlationId);
        var client = Harness.GetRequestClient<DepositWithdrawFailedNotification>();

        _transactionQueriesV2.Setup(q => q.GetTransactionById(correlationId)).ReturnsAsync(() => null);

        // Act
        var exception = await Assert.ThrowsAsync<RequestFaultException>(async () => await client.GetResponse<DepositWithdrawFailedNotificationResponse>(payload));
    ...
    public async void Should_Be_Success_When_TransactionType_And_Product_Correctly(TransactionType transactionType, Product? product)
    {
        ...
        // Act
        await client.GetResponse<DepositWithdrawFailedNotificationResponse>(payload);

        // Assert
        Assert.True(await Harness.Consumed.Any<DepositWithdrawFailedNotificationResponse>());
    }

    [Theory]
    [InlineData(TransactionType.Deposit, Product.CashBalance, 30)]
    [InlineData(TransactionType.Deposit, Product.CreditBalanceSbl, 31)]
    [InlineData(TransactionType.Deposit, Product.Cash, 32)]
    [InlineData(TransactionType.Deposit, Product.Derivatives, 33)]
    [InlineData(TransactionType.Deposit, Product.GlobalEquities, 49)]
    [InlineData(TransactionType.Withdraw, Product.CashBalance, 25)]
    [InlineData(TransactionType.Withdraw, Product.CreditBalanceSbl, 25)]
    [InlineData(TransactionType.Withdraw, Product.Cash, 25)]
    [InlineData(TransactionType.Withdraw, Product.Derivatives, 25)]
    [InlineData(TransactionType.Withdraw, Product.GlobalEquities, 25)]
    ...
    {
        // Arrange
        var consumer = new NotificationFailedConsumer(_logger.Object, _transactionQueriesV2.Object, _notificationService.Object);
