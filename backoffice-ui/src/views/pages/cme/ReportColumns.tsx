...
const onClickDownload = async (reportId: string, reportName: string) => {
  try {
...
export const columns: GridColDef[] = [
          ...
          <Tooltip title='Download'>
            <span>
              <IconButton
                ...
                onClick={() => onClickDownload(params?.row?.id, params?.row?.name)}
              >
  ...
  },
  {
    ...
    field: 'version',
    sortable: false,
    renderCell: (params: GridRenderCellParams) => renderStatus(params.row.version),
  },
  {
    flex: 0.15,
    type: 'date',
    ...
    renderCell: (params: GridRenderCellParams) => (
      <Typography variant='body2' sx={{ color: 'text.primary' }}>
        {params.row.dateFrom ? `${dayjs(params.row.dateFrom).format('DD/MM/YYYY')}` : '-'}
      </Typography>
    ),
  },
]
