...
interface Props {
  ...
  onSearchChange: (searchValue: { symbol: string }) => void
}
...
const DashboardAction = ({ onSubmit, onSearchChange }: Props) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <TextField
                    ...
                    label='Symbol'
                    placeholder='Symbol'
                    aria-describedby='validation-schema-symbol'
                    error={Boolean(errors.symbol)}
                    onChange={e => {
                      onChange(e)
                      onSearchChange({ symbol: e.target.value })
                    }}
                  />
                )}
              />
