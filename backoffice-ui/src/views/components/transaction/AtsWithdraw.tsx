import { IAtsWithdrawDetail } from '@/lib/api/clients/backoffice/transactions/types'
...
const AtsWithdrawDetail = ({ data }: Props) => {
  const [rows, setRows] = useState<IAtsWithdrawDetail[]>([])
  ...
  const renderRows = (rows: IAtsWithdrawDetail[]) =>
    rows?.map((row: IAtsWithdrawDetail) => [
      { value: row?.state ?? '-' },
      {
  ...
  const renderTable = (rows: IAtsWithdrawDetail[]) => {
  ...
  return (
    <div>
      <Card sx={{ marginTop: '40px' }}>
        <CardHeader title='ATS Withdraw Detail'></CardHeader>
        {renderTable(rows)}
      </Card>
    </div>
  )
...
export default AtsWithdrawDetail
