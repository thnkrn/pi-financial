import { IOddDepositDetail } from '@/lib/api/clients/backoffice/transactions/types'
...
const OddDepositDetail = ({ data }: Props) => {
  const [rows, setRows] = useState<IOddDepositDetail[]>([])
  ...
  const renderRows = (rows: IOddDepositDetail[]) =>
    rows?.map((row: IOddDepositDetail) => [
      { value: row?.state ?? '-' },
      {
  ...
  const renderTable = (rows: IOddDepositDetail[]) => {
  ...
  return (
    <div>
      <Card sx={{ marginTop: '40px' }}>
        <CardHeader title='ODD Deposit Detail'></CardHeader>
        {renderTable(rows)}
      </Card>
    </div>
  )
...
export default OddDepositDetail
