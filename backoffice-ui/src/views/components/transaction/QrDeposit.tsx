import { IQrDepositDetail } from '@/lib/api/clients/backoffice/transactions/types'
...
const QrDepositDetail = ({ data }: Props) => {
  const [rows, setRows] = useState<IQrDepositDetail[]>([])
  ...
  const renderRows = (rows: IQrDepositDetail[]) =>
    rows?.map((row: IQrDepositDetail) => [
      { value: row?.state ?? '-' },
      {
  ...
  const renderTable = (rows: IQrDepositDetail[]) => {
  ...
  return (
    <div>
      <Card sx={{ marginTop: '40px' }}>
        <CardHeader title='QR Deposit Detail'></CardHeader>
        {renderTable(rows)}
      </Card>
    </div>
  )
...
export default QrDepositDetail
