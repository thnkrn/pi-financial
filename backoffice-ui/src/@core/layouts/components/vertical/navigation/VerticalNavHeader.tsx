...
const VerticalNavHeader = (props: Props) => {
  ...
  const svgRotationDeg = () => {
    ...
    if (navCollapsed) {
      if (navHover) {
        rotation = direction === 'rtl' ? 0 : 180
      } else {
        rotation = direction === 'rtl' ? 180 : 0
      }
    } else {
      rotation = direction === 'rtl' ? 180 : 0
    }
  ...
  return (
    <MenuHeaderWrapper className='nav-header' sx={{ pl: menuHeaderPaddingLeft() }}>
      {userNavMenuBranding ? (
        ...
        <LinkStyled href='/'>
          <Image src='/images/pi-logo.svg' alt='pi-logo' width={50} height={50} style={{ width: 'auto', height: 'auto' }} priority/>
