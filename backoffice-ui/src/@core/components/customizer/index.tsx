...
interface ColorBoxProps {
  color: Settings[keyof Settings]
  backgroundColor: string
  themeColor: ThemeColor
  onClick: () => void
}

const RenderColorBox = ({ color, backgroundColor, themeColor, onClick }: ColorBoxProps) => (
  <ColorBox
    onClick={onClick}
    sx={{
      ml: color === 'primary' ? 0 : null,
      mr: color === 'info' ? 0 : null,
      backgroundColor,
      ...(themeColor === color ? { boxShadow: 9 } : { '&:hover': { boxShadow: 4 } }),
    }}
  >
    {themeColor === color ? <Icon icon='mdi:check' fontSize='1.25rem' /> : null}
  </ColorBox>
)
...
const COLOR_PICKER: IColorPicker[] = [
  {
    color: 'primary',
        ...
        <PerfectScrollbar options={{ wheelPropagation: false }}>
          <CustomizerSpacing className='customizer-body'>
            ...
            <div>
              ...
              <Box sx={{ display: 'flex' }}>
                {COLOR_PICKER.map((colorPicker: IColorPicker) => (
                  <RenderColorBox
                    key={colorPicker.backgroundColor}
                    color={colorPicker.color}
                    backgroundColor={colorPicker.backgroundColor}
                    themeColor={themeColor}
                    onClick={() => handleChange('themeColor', colorPicker.color)}
                  />
                ))}
              </Box>
            </div>
