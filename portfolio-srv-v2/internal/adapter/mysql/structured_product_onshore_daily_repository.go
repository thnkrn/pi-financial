...
type portfolioStructuredProductOnshoreDailySnapshot struct {
...
type StructuredProductOnshoreDailyRepository struct {
	logger log.Logger
	db     *gorm.DB
}

func NewStructuredProductOnshoreDailyRepository(logger log.Logger, db *gorm.DB) *StructuredProductOnshoreDailyRepository {
	return &StructuredProductOnshoreDailyRepository{
		logger: logger,
		db:     db,
	}
}

func (r *StructuredProductOnshoreDailyRepository) GetByCustomerCodeWithLatestDateKey(ctx context.Context, customerCode string) ([]model.StructuredProductSummary, error) {
	subQuery := r.db.Table("portfolio_structured_product_onshore_daily_snapshot").
		Select("MAX(date_key)").
		Where("custcode = ?", customerCode)

	var dbSnapshots []portfolioStructuredProductOnshoreDailySnapshot
	err := r.db.Table("portfolio_structured_product_onshore_daily_snapshot").
		WithContext(ctx).
...
func (g *StructuredProductOnshoreDailyRepository) mapToPublicModel(s portfolioStructuredProductOnshoreDailySnapshot) model.StructuredProductSummary {
